...
```bash
FORCE_HYPERLINK=1 agy
```

Click **Click here to authenticate**, approve in the browser, copy the code it
shows, paste it into the `authorization code...` field, press Enter, then leave
with `/quit`.

> **Tip:**
>
> `FORCE_HYPERLINK=1` is what makes that a single clickable link. This terminal
> runs inside tmux, and `agy` prints a plain wrapped URL when it sees that. The
> wrap is hard, so each row is its own fragment: clicking one sends half a URL and
> Google answers `Error 400 (Bad Request)`, `invalid_request`.
